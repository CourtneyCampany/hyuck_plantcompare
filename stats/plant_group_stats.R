...
##model has species as random effect


#stomatal density
stomden_mod <- lmer(sto_den ~ plant_group + (1|species), data=stom)
visreg(stomden_mod)
...
tukey_stom <- glht(stomden_mod, linfct = mcp(plant_group = "Tukey"))
stom_siglets <-cld(tukey_stom)
stom_siglets2 <- stom_siglets$mcletters$Letters

#photosynthesis
photo_mod <- lmer(Photo ~ plant_group + (1|species), data=photo)
visreg(photo_mod)
...
tukey_photo <- glht(photo_mod, linfct = mcp(plant_group = "Tukey"))
photo_siglets <-cld(tukey_photo)
photo_siglets2 <- photo_siglets$mcletters$Letters

#nitrogen
nitro_mod <- lmer(n_perc ~ plant_group + (1|species), data=photo)
visreg(nitro_mod)
...
tukey_nitro <- glht(nitro_mod, linfct = mcp(plant_group = "Tukey"))
nitro_siglets <-cld(tukey_nitro)
nitro_siglets2 <- nitro_siglets$mcletters$Letters

#stomatal conductance
cond_mod <- lmer(Cond ~ plant_group + (1|species), data=photo)
visreg(cond_mod)
...
tukey_cond <- glht(cond_mod, linfct = mcp(plant_group = "Tukey"))
...
wue_mod <- lmer(iwue ~ plant_group + (1|species), data=wuedat)
visreg(wue_mod)
...
tukey_wue <- glht(wue_mod, linfct = mcp(plant_group = "Tukey"))
