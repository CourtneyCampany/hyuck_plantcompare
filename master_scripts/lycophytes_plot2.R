...
chem <- read.csv("raw_data/photo_chem.csv")
  chem$cnratio <- with(chem, c_perc/n_perc)
  chem$wue <- with(chem, Photo/Trmmol)

phos <- chem[chem$p_perc > .1 & chem$p_perc < .4,]


##panel plot with non-lycos in both habitats
windows()
par(mfrow=c(2,2), mgp=c(3,.75,0))

#N content
par(mar=c(0,5.5,1,0))
boxplot(n_perc ~ plant_group, data=chem, ylab="Foliar N content  (%)",
        col=pgcols, ylim=c(0,5), xaxt='n', yaxt='n', outline=FALSE)
axis(2, las=1)

#P content
par(mar=c(0,0,1,5.5))
boxplot(p_perc ~ plant_group, data=phos, ylab="",outline=FALSE,
...
boxplot(cnratio ~ plant_group, data=chem, ylab="Foliar C:N ratio",yaxt='n',
        names=plants,col=pgcols, ylim=c(0,45), outline=FALSE)
axis(2, las=1)

#water use efficiency
par(mar=c(5.5,0,0,5.5))
boxplot(wue ~ plant_group, data=chem, ylab="",names=plants,
        col=pgcols, ylim=c(0,21), outline=FALSE, yaxt='n')
axis(4,las=1)
mtexti(wuelab, 4, outer=TRUE, cex=1, off=.6)

dev.copy2pdf(file= "output/lycophytes2.pdf")
dev.off()
